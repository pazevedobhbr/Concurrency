...
func (app *Config) SubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		msg := Message{
			...
			AttachmentMap: map[string]string{
				"manual.pdf": fmt.Sprintf("%sp/%d_manual.pdf", tmpPath, user.ID),
			},
		}
		app.sendEmail(msg)

		//test app error channel
	...
	//redirect
	app.Session.Put(r.Context(), "flash", "You have subscribed to a plan. Check your email for invoice and manual.")
	http.Redirect(w, r, "/members/plans", http.StatusSeeOther)
}
...
func (app *Config) generateManual(u data.User, plan *data.Plan) *gofpdf.Fpdf {
	...
	pdf.MultiCell(0, 4, fmt.Sprintf("Name: %s %s", u.FirstName, u.LastName), "", "C", false)
	pdf.Ln(5)
	pdf.MultiCell(0, 4, fmt.Sprintf("%s UserGuide", plan.PlanName), "", "C", false)
